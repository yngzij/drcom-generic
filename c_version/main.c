 ...
 *  这是一个MOD的版本，也就是参考了以下Latyas
 * <latyas@gmail.com>的两部分共同完成的一个版本
 ...
 *
 *  这个版本理论上适用于绝大多数D版，修改d_generic＝1再改下方配置,当d_general
 * =0时为cust专用
 *
 *  修正LOGIN包对齐问题，修正KL2计数问题,离线测试参数offlinetest，在线使用请置0
 *
 *  整体框架移植包含了KEEP ALIVE
 * 2部分，但是我们学校并不对其进行校验，所以具体工作与否暂未验证,
 *
...
uint64_t mac = 0x6036ddeabcde; // echo 0x`ifconfig eth | egrep -io
                               // "([0-9a-f]{2}:){5}[0-9a-f]{2}" | tr -d ":"`
...
void set_challenge_data(unsigned char *clg_data, int clg_data_len,
  ...
  clg_data[data_index++] =
      0x02 + (unsigned char)clg_try_count; // python里面这一段是只用02
...
void challenge(int sock, struct sockaddr_in serv_addr, unsigned char *clg_data,
  ...
  do {
    if (challenge_try > CHALLENGE_TRY) {
      close(sock);
      fprintf(stderr, "[drcom-challenge]: try challenge, but failed, please "
                      "check your network connection.\n");
      exit(EXIT_FAILURE);
    } //当challenge尝试超过10次预设值时，释放套接口描述字s，报错
    set_challenge_data(clg_data, clg_data_len,
                       challenge_try); //生成challenge_dat数据
    if (INTERFACE == 1)
      printf("[debug]challenge packet make !\n");
    fprintf(stderr, "%s", clg_data);
    challenge_try++; //尝试次数自加
    ret = sendto(
        sock, clg_data, clg_data_len, 0, (struct sockaddr *)&serv_addr,
        sizeof(
            serv_addr)); //将字符串发送给server,,,ret=sendto(已建立的连接，clg包，clg包长度，flags设0不变，sockaddr结构体，前者长度)
    if (ret != clg_data_len) { // ret不等于clg_data长度报错
      fprintf(stderr, "[drcom-challenge]: send challenge data failed.\n");
      continue;
    }
    ret = recvfrom(sock, recv_data, recv_len, 0, NULL,
                   NULL); // ret=server返回的字符串
    if (ret < 0) {        //返回值小于0报错
      fprintf(stderr, "[drcom-challenge]: recieve data from server failed.\n");
      continue;
    }
    if (*recv_data !=
        0x02) { // receive_data第一个字符不为02，但为07报错退出，否则继续循环
      if (*recv_data == 0x07) {
        close(sock);
...
void set_login_data(struct user_info_pkt *user_info, unsigned char *login_data,
                    int login_data_len, unsigned char *salt,
                    int salt_len) //这句里定义了salt是challenge返回值4-8位
{
  /* login data */
  int i, j;
  ...
  login_data[data_index++] =
      (unsigned char)(user_info->username_len +
                      20); // ASCII值转字符串（字符串长度（账号）+20）

  /* md5 0x03 0x01 salt password 16位*/
  md5_str_len =
      2 + salt_len + user_info->password_len; // MD5_1 md5_str长度为2+4+密码长度
  memset(md5_str_tmp, 0x00,
         md5_str_len); //将md5_str_tmp的前md5_str_len位全部置0x00
  md5_str_tmp[0] = 0x03;
  md5_str_tmp[1] = 0x01;
  memcpy(md5_str_tmp + 2, salt, salt_len); //++salt
  memcpy(md5_str_tmp + 2 + salt_len, user_info->password,
         user_info->password_len); //++password
  MD5(md5_str_tmp, md5_str_len, md5_str);
  memcpy(login_data + data_index, md5_str,
         16); //向data_index中添加16位的md5_str
  data_index += 16;

  /* user name 36位 */
  memcpy(login_data + data_index, user_info->username, user_info->username_len);
  data_index +=
      36; //用户名ASCII码先填入logindata+data_index偏移量位置，然后强艹data_index为36
  ...
  for (i = 6; i > 0; i--) {
    login_data[data_index + i - 1] =
        (unsigned char)(sum % 256); //把sum逐位加回login_data中，共6位
    sum /= 256;
  }
  data_index += 6;

  /* md5 0x01 pwd salt 0x00 0x00 0x00 0x00 */
  md5_str_len = 1 + user_info->password_len + salt_len +
                4; // MD5_2    长度为1+密码长度+salt长度+4个零位
  memset(md5_str_tmp, 0x00, md5_str_len); // MD5temp区前md5_str_len清零
  md5_str_tmp[0] = 0x01;                  //第一位01
  memcpy(md5_str_tmp + 1, user_info->password,
         user_info->password_len); //拷贝密码
  memcpy(md5_str_tmp + 1 + user_info->password_len, salt, salt_len); //拷贝salt
  MD5(md5_str_tmp, md5_str_len, md5_str); //计算MD5_2,赋值到md5_str
  memcpy(login_data + data_index, md5_str,
         16);       //拷贝16位的MD5_2到logindata+偏移量
  ...
  memcpy(md5_str_tmp, login_data,
         data_index); //把前面logindata全部内容拷贝到md5_str_tmp
  ...
  /* hostname */
  i = user_info->hostname_len > 32
          ? 32
          : user_info->hostname_len; //这一段拷贝Hostname,用i表示长度，偏移量+32
  ...
  /* checksum point 4*/
  int check_point =
      data_index; //定义check_point为偏移量，方便一会儿覆盖数据，然后给logindata加'\x01\x26\x07\x11\x00\x00'+dump(mac)
  login_data[data_index++] = 0x01;
  ...
  for (j = 0; j < 4; j++) {
    login_data[check_point + j] =
        (unsigned char)(sum >> (j * 8) &
                        0x000000FF); //把4位checksum结果写入check_point开始的logindata处覆盖数据
  }
  /* 0x00 0x00 0x00 0x00 */
  login_data[data_index++] = 0x00;
  login_data[data_index++] = 0x00;
...
void login1(int sock, struct sockaddr_in serv_addr, unsigned char *login_data,
  ...
  do {
    if (login_try > LOGIN_TRY) {
      close(sock);
      fprintf(stderr, "[drcom-login]: try login, but failed, something "
                      "wrong.\n"); //尝试次数过多报错
      exit(EXIT_FAILURE);
    }
    login_try++;
    ret = sendto(sock, login_data, login_data_len, 0,
                 ...
                 sizeof(serv_addr)); // ret=发送到服务器的数据长度
    ...
    if (*recv_data !=
        0x04) { //如果接收数据首字符不为04，且为05，报错提示账号或密码错误
      if (*recv_data == 0x05) {
        close(sock);
...
void set_alive1_data(unsigned char *alive_data1, int alive_data1_len,
                     unsigned char *tail, unsigned char *salt, int salt_len,
                     struct user_info_pkt *user_info, unsigned char *md5_str)
// set_alive_data1(send_data, alive1_data_len, package_tail,
// globle_salt,4,&user_info);
{
  ...
  *timep = time(NULL);
  ...
  // memcpy(md5_str_tmp + j + salt_len, user_info->password,
  // user_info->password_len); MD5(md5_str_tmp, md5_str_len, md5_str);
  //}
...
void set_alive2_data(unsigned char *alive_data2, int alive_data2_len,
                     unsigned char *tail, int tail_len, int alive_count,
                     int type, unsigned char *kl1_recv_data, int first)
...
void logout1(int sock, struct sockaddr_in serv_addr, unsigned char *logout_data,
             int logout_data_len, char *recv_data, int recv_len) {
...
void logout_signal(int signum) { logout_flag = 1; }
...
int main(int argc, char **argv) {
  ...
  // long ret;
  unsigned char
      send_data[SEND_DATA_SIZE]; //定义无符号字符串send_data[1000]  长度为1000
  ...
  sock = socket(
      AF_INET, SOCK_DGRAM,
      0); // AF_INET决定了要用ipv4地址（32位的）与端口号（16位的）的组合，。数据报式Socket（SOCK_DGRAM）是一种无连接的Socket，对应于无连接的UDP服务应用
  if (sock < 0) { // sock<0即错误
  ...
  // int mtype=0; //定义整形变量mtype=0 keep_alive_1((unsigned char
  // *)&send_data,(unsigned char *)&recv_data,alive1_data_len,(unsigned char
  // *)&package_tail,(unsigned char
  // *)&globle_salt,&user_info,&serv_addr,globle_md5a,sock,mtype);
  do {
    set_alive1_data(send_data, alive1_data_len, package_tail, globle_salt, 4,
                    &user_info, globle_md5a);
    memset(recv_data, 0x00, RECV_DATA_SIZE);
    if (OFF_LINE_TEST == 0)
      sendto(sock, send_data, 38, 0, (struct sockaddr *)&serv_addr,
             sizeof(serv_addr));
    ...
    else if (*recv_data == 0x07) //如果接受数据首字符为07，显示ka1成功
    {
      ...
      while (recv_data[5] < 4) {
        ...
        if (D_GENERIC == 1) // D_GENERIC
        {
          ...
          set_alive2_data(send_data, alive2_data_len, package_tail_empty, 4,
                          alive_count, 1, (unsigned char *)recv_data, 1);
          //(unsigned char *alive_data2, int alive_data2_len, unsigned char
          //*tail, int tail_len, int alive_count,int type,unsigned char
          //*kl1_recv_data,int first)
          memset(recv_data, 0x00, RECV_DATA_SIZE);
          if (OFF_LINE_TEST == 0) {
            sendto(sock, (char *)&send_data, alive2_data_len, 0,
          ...
          // 02
          set_alive2_data(send_data, alive2_data_len, package_tail_empty, 4,
                          alive_count, 1, (unsigned char *)recv_data, 0);
          memset(recv_data, 0x00, RECV_DATA_SIZE);
          if (OFF_LINE_TEST == 0)
            sendto(sock, (char *)&send_data, alive2_data_len, 0,
                   (struct sockaddr *)&serv_addr, sizeof(struct sockaddr));
          ...
          // 03
          set_alive2_data(send_data, alive2_data_len, package_tail2, 4,
                          alive_count, 3, (unsigned char *)recv_data, 0);
          if (OFF_LINE_TEST == 0)
            sendto(sock, (char *)&send_data, alive2_data_len, 0,
                   (struct sockaddr *)&serv_addr, sizeof(struct sockaddr));
          printf("\n[keep-alive2]send3");
          ...
          do {
            printf("\n[DEBUG]LOOPING........");
            set_alive2_data(send_data, alive2_data_len, package_tail2, 4,
                            alive_count2, 1, (unsigned char *)recv_data, 0);
            if (OFF_LINE_TEST == 0)
              sendto(sock, (char *)&send_data, alive2_data_len, 0,
                     (struct sockaddr *)&serv_addr, sizeof(struct sockaddr));
            printf("\n[keep-alive2]send1");
            ...
            set_alive2_data(send_data, alive2_data_len, package_tail2, 4,
                            alive_count2 + 1, 3, (unsigned char *)recv_data, 0);
            if (OFF_LINE_TEST == 0)
              sendto(sock, (char *)&send_data, alive2_data_len, 0,
                     (struct sockaddr *)&serv_addr, sizeof(struct sockaddr));
            printf("\n[keep-alive2]send2");
            ...
            set_alive1_data(send_data, alive1_data_len, package_tail2,
                            globle_salt, 4, &user_info, globle_md5a);
            memset(recv_data, 0x00, RECV_DATA_SIZE);
            if (OFF_LINE_TEST == 0)
              sendto(sock, send_data, 38, 0, (struct sockaddr *)&serv_addr,
                     sizeof(serv_addr));
        ...
        {
          set_alive2_data(send_data, alive2_data_len, package_tail, 16,
                          alive_count, mtype, (unsigned char *)recv_data, 0);
          if (INTERFACE == 1)
            printf("\n[keep-alive-misc] ");
          if ((sendto(sock, (char *)&send_data, alive2_data_len, 0,
                      (struct sockaddr *)&serv_addr,
          ...
          if ((alive2_data_len = (int)recvfrom(sock, (char *)&recv_data,
                                               RECV_DATA_SIZE, 0, NULL, NULL)) <
              0)
            exit(errno);
