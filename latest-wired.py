...
def keep_alive2(*args):
    ...
    ran += random.randint(1, 10)
    packet = keep_alive_package_builder(
        svr_num, dump(ran), '\x00' * 4, 1, False)
...
def mkpkt(salt, usr, pwd, mac):
    ...
    data += ADAPTERNUM
    data += dump(int(data[4:10].encode('hex'), 16) ^
                 mac).rjust(6, '\x00')  # mac xor md51
    ...
    if ror_version:
        ...
        data += ror(md5sum('\x03\x01' + salt + pwd),
    ...
    data += checksum(data + '\x01\x26\x07\x11\x00\x00' +
...
def keep_alive1(salt, tail, pwd, svr):
    if keep_alive1_mod:
        res = ''
        while True:
            s.sendto('\x07' + struct.pack('!B', int(time.time()) %
                                          0xFF) + '\x08\x00\x01\x00\x00\x00', (svr, 61440))
        ...
        data = '\xFF' + '\x00' * 7 + seed + crc + tail + \
            struct.pack('!H', int(time.time()) % 0xFFFF)
        ...
        while True:
            try:
                ...
                log('[keep_alive1] error',
                    'raise Exception to main() or keep_alive2()')
                raise
...
def main():
    ...
    while True:
        ...
        try:
            ...
            print(e)
            log('[main] error',
                'something was wrong in keep_alives, do everything again')
